...
public class L1DragonSlayer {
    ...
    public class DragonSlayerTimer extends TimerTask {
        ...
        @Override
        public void run() {
            final int mapId = (1005 + this._num);
            ...
            switch (this._status) {
                ...
                case STATUS_DRAGONSLAYER_READY_4RD:
                    ...
                    if ((this._num >= 0) && (this._num <= 5)) {
                        L1DragonSlayer.this.spawn(71014, this._num, 32783,
                                32693, mapId, 10, 0); // 地龙 - 阶段一
                    } else {
                        L1DragonSlayer.this.spawn(71026, this._num, 32955,
                ...
                case STATUS_DRAGONSLAYER_START_2RD_3:
                    ...
                    // 召唤龙
                    if ((this._num >= 0) && (this._num <= 5)) {
                        L1DragonSlayer.this.spawn(71015, this._num, 32783,
                                32693, mapId, 10, 0); // 地龙 - 阶段二
                    } else {
                        L1DragonSlayer.this.spawn(71027, this._num, 32955,
                ...
                case STATUS_DRAGONSLAYER_START_3RD_2:
                    ...
                    // 召唤龙
                    if ((this._num >= 0) && (this._num <= 5)) {
                        L1DragonSlayer.this.spawn(71016, this._num, 32783,
                                32693, mapId, 10, 0); // 地龙 - 阶段三
                    } else {
                        L1DragonSlayer.this.spawn(71028, this._num, 32955,
                ...
                case STATUS_DRAGONSLAYER_END_2:
                    ...
                    if (L1DragonSlayer.this.checkHiddenDragonValleyStstus() == STATUS_READY_SPAWN) { // 开启隐匿的巨龙谷入口
                        ...
                        L1DragonSlayer.this.spawn(81277, -1, 33726, 32506, 4,
                                0, 86400000); // 24小时
                    }
                ...
                case STATUS_DRAGONSLAYER_END_5:
                    ...
                    if (L1DragonSlayer.this.portalPack()[this._num] != null) {
                        ...
                        L1DragonSlayer.this.portalPack()[this._num]
                                .broadcastPacketAll(new S_DoActionGFX(
                                        L1DragonSlayer.this.portalPack()[this._num]
                                                .getId(),
    ...
    public void resetDragonSlayer(final int portalNumber) {
        final int mapId = (1005 + portalNumber); // MapId 判断

        for (final Object obj : World.get().getVisibleObjects(mapId)
                .values()) {
            // 将玩家传出副本地图
    ...
    public void sendMessage(final int portalNum, final int type,
            final String msg) {
        final int mapId = (1005 + portalNum);
    ...
    public void spawn(final int npcId, final int portalNumber, final int X,
            final int Y, final int mapId, final int randomRange,
        ...
        try {
            ...
            if (randomRange == 0) {
                ...
                do {
                    tryCount++;
                    npc.setX(X + _random.nextInt(randomRange)
                            - _random.nextInt(randomRange));
                    npc.setY(Y + _random.nextInt(randomRange)
                            - _random.nextInt(randomRange));
                    if (npc.getMap().isInMap(npc.getLocation())
                            && npc.getMap().isPassable(npc.getLocation(), null)) {
                        break;
                    }
                    Thread.sleep(1);
                } while (tryCount < 50);
            ...
            if ((npc.getGfxId() == 7548) || (npc.getGfxId() == 7550)
                    ...
                    || (npc.getGfxId() == 7870)) {
                npc.calcSleepTime(ActionCodes.ACTION_AxeWalk,
            ...
            if (0 < timeMillisToDelete) {
                final NpcDeleteTimer_1 timer = new NpcDeleteTimer_1(npc,
                        timeMillisToDelete);
                timer.begin();
            }
