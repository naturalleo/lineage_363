...
public class L1TownLocation {
    ...
    public static final boolean isGambling(final int x, final int y,
            final short map) {
    ...
    public static int[] getGetBackLoc(final int town_id) { // town_idから归还先の座标をランダムに返す
        ...
        switch (town_id) {
            ...
            case TOWNID_SILENT_CAVERN: // 沈默の洞窟
            ...
            case TOWNID_RECLUSE_VILLAGE: // 隐れ里
    ...
    public static int getTownIdByNpcid(final int npcid) { // npcidからtown_idを返す
        ...
        // XXX:まだNPCはL1CastleLocationから持ってきたままの状态（未整理）
        int town_id = 0;

        switch (npcid) {
            case 70528: // タウンマスター（TI）
            case 50015: // ルーカス（テレポーター）
            case 70010: // バルシム（犬小屋里道具屋）
            case 70011: // 船着场管理人
            case 70012: // セレナ（宿屋）
