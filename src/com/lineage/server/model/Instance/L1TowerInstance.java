...
public class L1TowerInstance extends L1NpcInstance {
    ...
    public void receiveDamage(final L1Character attacker, final int damage) {
        ...
        if ((_castle_id > 0) && ServerWarExecutor.get().isNowWar(_castle_id)) {
            ...
            if ((_castle_id == L1CastleLocation.ADEN_CASTLE_ID)
                    && !isSubTower()) {
                int subTowerDeadCount = 0;// 子塔被破坏数量
        ...
        if (isDamage) {
            ...
            } else if (attacker instanceof L1PetInstance) {// 攻击者是宠物
                pc = (L1PcInstance) ((L1PetInstance) attacker).getMaster();

            } else if (attacker instanceof L1SummonInstance) {// 攻击者是 召换兽
                pc = (L1PcInstance) ((L1SummonInstance) attacker).getMaster();

            } else if (attacker instanceof L1IllusoryInstance) {// 攻击者是 分身
                pc = (L1PcInstance) ((L1IllusoryInstance) attacker).getMaster();

            } else if (attacker instanceof L1EffectInstance) {// 攻击者是 技能物件(火牢)
                // 火牢无法伤害守护塔
            ...
            boolean existDefenseClan = false;// 该城堡是否具有城盟
            ...
            boolean isProclamation = false;// 是否为战争中血盟
            // 全部战争资料
            for (final L1War war : WorldWar.get().getWarList()) {
                if (_castle_id == war.getCastleId()) { // 攻击的城堡在战争状态
    ...
    private class Death implements Runnable {
        ...
        private L1Character _lastattacker;// 攻击者
