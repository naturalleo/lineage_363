...
public class EffectCubeHarmonizeTimer extends TimerTask {
    ...
    private static final Log _log = LogFactory
            .getLog(EffectCubeHarmonizeTimer.class);
    ...
    public void run() {
        try {
            ...
            /*
             * for (final L1EffectInstance effect : allNpc) { // 不是幻术师技能(立方：和谐)
             * if (effect.effectType() != L1EffectType.isCubeHarmonize) {
             * continue; } // 计算结果 EffectCubeExecutor.get().cubeBurn(effect);
             * Thread.sleep(1); }
             */

        } catch (final Exception e) {
            _log.error("Npc L1Effect幻术师技能(立方：和谐)状态送出时间轴异常重启", e);
