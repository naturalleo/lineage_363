...
/**
 * <font color=#00800>三星對武器施法的卷軸</font><BR>
 *
 */
public class ScrollEnchantWeapon_3xing extends ItemExecutor {

	/**
	 *
	 */
	private ScrollEnchantWeapon_3xing() {
		// TODO Auto-generated constructor stub
	}

	public static ItemExecutor get() {
		return new ScrollEnchantWeapon_3xing();
	}
	...
	public void execute(final int[] data, final L1PcInstance pc, final L1ItemInstance item) {
		// 對象OBJID
		if (ConfigOther.XING_COUNT < 3) { //最大开放星数 hjx1000
			final int itemid = item.getItemId() - 1;
			pc.getInventory().removeItem(item, 1);
			CreateNewItem.createNewItem(pc, itemid, 10);
			return;
		}
		...
		case 1:// 武器
			...
			if (power_name != null && tgItem.get_power_name().get_xing_count() > 2) { //三星武器		
				is3xing = true;
				if (tgItem.get_power_name().get_xing_count() > 3) {//武器星数 大于3不能强化hjx1000
					isErr = true;
				}				
			}
			break;
		...
		if (enchant_level < -6) {// 武器将会消失,最大可追加到-7
			...
			if (!is3xing && rnd3 < Super) {//星武器成功机率 hjx1000
				pc.sendPackets(new S_ServerMessage(161, s.toString(), "耀眼的", "$247"));
				L1ItemPower_name power = null;
				if (tgItem.get_power_name() != null) {
					final int i = tgItem.get_power_name().get_xing_count();
					tgItem.get_power_name().set_xing_count(i+1);
				}else {
