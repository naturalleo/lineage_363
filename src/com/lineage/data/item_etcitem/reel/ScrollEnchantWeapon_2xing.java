...
/**
 * <font color=#00800>二星對武器施法的卷軸</font><BR>
 *
 */
public class ScrollEnchantWeapon_2xing extends ItemExecutor {

	/**
	 *
	 */
	private ScrollEnchantWeapon_2xing() {
		// TODO Auto-generated constructor stub
	}

	public static ItemExecutor get() {
		return new ScrollEnchantWeapon_2xing();
	}
	...
	public void execute(final int[] data, final L1PcInstance pc, final L1ItemInstance item) {
		if (ConfigOther.XING_COUNT < 2) { //最大开放星数 hjx1000
		...
		case 1:// 武器
			...
			if (power_name != null && tgItem.get_power_name().get_xing_count() > 1) { //二星武器		
				is2xing = true;
				if (tgItem.get_power_name().get_xing_count() > 2) {//武器星数 大于2不能强化hjx1000
					isErr = true;
				}				
			}
			break;
		...
		if (enchant_level < -6) {// 武器将会消失,最大可追加到-7
			...
			if (!is2xing && rnd3 < Super) {//星武器成功机率 hjx1000
