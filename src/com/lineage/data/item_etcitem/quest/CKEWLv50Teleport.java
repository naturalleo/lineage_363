            ...
            for (L1Object tgobj : mapList.values()) {
                if (tgobj instanceof L1PcInstance) {
                    ...
                    if (tgpc.get_showId() == pc.get_showId()) {
                        ...
                        if (tgpc.isCrown()) {// 王族
                            // 传送任务执行者
                            L1Teleport.teleport(tgpc, 32790, 32773,
                                    (short) CKEWLv50_1.MAPID, 2, true);

                        } else if (tgpc.isKnight()) {// 骑士
                            // 传送任务执行者
                            L1Teleport.teleport(tgpc, 32788, 32773,
                                    (short) CKEWLv50_1.MAPID, 2, true);

                        } else if (tgpc.isElf()) {// 精灵
                            // 传送任务执行者
                            L1Teleport.teleport(tgpc, 32788, 32771,
                                    (short) CKEWLv50_1.MAPID, 2, true);

                        } else if (tgpc.isWizard()) {// 法师
                            // 传送任务执行者
                            L1Teleport.teleport(tgpc, 32788, 32775,
                                    (short) CKEWLv50_1.MAPID, 2, true);
                        }
                    }
                }
