...
/**
 * 武器致命魔法卷轴lv2 55112<br>
 */
public class Deadly_Lv2 extends ItemExecutor {
    ...
    public void execute(final int[] data, final L1PcInstance pc,
        ...
        boolean isErr = false;
        if (power_name == null) {
        	pc.sendPackets(new S_SystemMessage("这个卷轴只可以强化Lv1的武器。"));
        	return;
        }
        if (power_name.get_xing_count() != 1) {
        	pc.sendPackets(new S_SystemMessage("这个卷轴只可以强化Lv1的武器。"));
        	return;
        }
        // 取得物件触发事件
        ...
        if (safe_enchant == 0 && enchant_level < 5) {
        	pc.sendPackets(new S_SystemMessage("你的武器必须要+5以上才可以使用这个卷轴。"));
        	return;
        }
        if (safe_enchant == 6 && enchant_level < 7) {
        	pc.sendPackets(new S_SystemMessage("你的武器必须要+7以上才可以使用这个卷轴。"));
        	return;
        }
        final Random random = new Random();
        pc.getInventory().removeItem(item, 1);                					
        tgItem.get_power_name().set_hole_1(random.nextInt(6) + 1);
		tgItem.get_power_name().set_xing_count(2);
