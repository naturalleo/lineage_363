...
/**
 * 武器致命魔法卷轴lv4 55114<br>
 */
public class Deadly_Lv4 extends ItemExecutor {
    ...
    public void execute(final int[] data, final L1PcInstance pc,
        ...
        boolean isErr = false;
        if (power_name == null) {
        	pc.sendPackets(new S_SystemMessage("这个卷轴只可以强化Lv3的武器。"));
        	return;
        }
        if (power_name.get_xing_count() != 3) {
        	pc.sendPackets(new S_SystemMessage("这个卷轴只可以强化Lv3的武器。"));
        	return;
        }
        // 取得物件触发事件
        ...
        if (safe_enchant == 0 && enchant_level < 7) {
        	pc.sendPackets(new S_SystemMessage("你的武器必须要+7以上才可以使用这个卷轴。"));
        	return;
        }
        if (safe_enchant == 6 && enchant_level < 9) {
        	pc.sendPackets(new S_SystemMessage("你的武器必须要+9以上才可以使用这个卷轴。"));
        	return;
        }
        final Random random = new Random();
        pc.getInventory().removeItem(item, 1);                   					
        tgItem.get_power_name().set_hole_2(random.nextInt(6) + 1);
		tgItem.get_power_name().set_xing_count(4);
