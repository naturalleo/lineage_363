...
public class Npc_DragonA1 extends NpcExecutor {
    ...
    public void talk(final L1PcInstance pc, final L1NpcInstance npc) {
        try {
            ...
            // 传送前往<font fg=66ff66><var src="#0"></font>执行副本条件限制(6,529：安塔瑞斯栖息地 )
    ...
    private void staraQuestA(L1PcInstance pc) {
        try {
                    ...
                    if (i <= (users - 1)) {
                        if (otherPc.getId() != party.getLeaderID()) {
                            ...
                            _playList.put(new Integer(otherPc.getId()),
                                    otherPc.getName());
                            // 传送成员
                            L1Teleport.teleport(otherPc, 32595, 32747,
                                    (short) mapid, 1, true);

                            // 将任务设置为执行中
                            QuestClass.get().startQuest(otherPc,
                                    ADLv80_1.QUEST.get_id());
                            // 将任务设置为结束
            ...
            L1SpawnUtil.spawnDoor(quest, 10005, 7556, 32759, 32799,
                    (short) mapid, 1);// A
            L1SpawnUtil.spawnDoor(quest, 10006, 7556, 32787, 32848,
                    (short) mapid, 1);// B
            L1SpawnUtil.spawnDoor(quest, 10007, 7556, 32781, 32919,
                    (short) mapid, 1);// C

            final L1Location loc = new L1Location(32781, 32705, mapid);
            L1SpawnUtil.spawn(71014, loc, 5, showId);// 新安塔瑞斯(1阶段)
            ...
            for (L1NpcInstance npc : quest.npcList()) {
                if (npc instanceof L1MonsterInstance) {
                    ...
                    if (mob.getNpcId() == 71014) {// 新安塔瑞斯(1阶段)
                        continue;
                    }
                    if (mob.getNpcId() == 71015) {// 新安塔瑞斯(2阶段)
                        continue;
                    }
                    if (mob.getNpcId() == 71016) {// 新安塔瑞斯(3阶段)
                        continue;
                    }
